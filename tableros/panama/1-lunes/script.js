...
function mostrarDatos() {
    ...
    onValue(ref(database, "tablero-panama-autopista-1-lunes"), (snapshot) => {
        const data = [];
...
form.addEventListener("submit", function (event) {
    ...
    if (!isSubmitting) {
        ...
        if (unidadInput.trim() !== "" && estadoInput.trim() !== "") {
            ...
            const referenciaUnidades = ref(database, "tablero-panama-autopista-1-lunes");
...
tabla.addEventListener("change", function (event) {
    if (event.target.classList.contains("estado-select")) {
        ...
        if (confirmar) {
            ...
            update(ref(database, `tablero-panama-autopista-1-lunes/${id}`), { estado: nuevoEstado })
                .then(() => {
...
confirmDeleteButton.addEventListener("click", function () {
    ...
    remove(ref(database, "tablero-panama-autopista-1-lunes"));
