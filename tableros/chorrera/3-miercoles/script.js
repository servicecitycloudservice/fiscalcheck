...
function mostrarDatos() {
    ...
    onValue(ref(database, "tablero-chorrera-miercoles"), (snapshot) => {
        const data = [];
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            const row = `
                        <tr>
                            ...
                            <option value="X" ${childData.estado === "X" ? "selected" : ""}>X</option>
...
form.addEventListener("submit", function (event) {
    ...
    if (!isSubmitting) {
        ...
        if (unidadInput.trim() !== "" && estadoInput.trim() !== "") {
            ...
            const referenciaUnidades = ref(database, "tablero-chorrera-miercoles");
...
tabla.addEventListener("change", function (event) {
    if (event.target.classList.contains("estado-select")) {
        ...
        if (confirmar) {
            ...
            update(ref(database, `tablero-chorrera-miercoles/${id}`), { estado: nuevoEstado })
                .then(() => {
...
confirmDeleteButton.addEventListener("click", function () {
    ...
    remove(ref(database, "tablero-chorrera-miercoles"));
